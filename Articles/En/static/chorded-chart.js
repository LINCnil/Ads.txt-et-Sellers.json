d3.json("static/data_chorded.json", function(error, data) {
    ...
    .attr("included-group", function(d){ return(["#value"+d.index])})
    .attr("d", arc)
    .style("opacity", 1);
    ...
    function highlight_node_links(node,i){
        ...
        d3.selectAll('.arc').transition().duration(200).style("opacity", 0.05);
        d3.selectAll('[source="'+selected+'"]').transition().duration(200).style("opacity", 1);
        //BIT OF A HACK TO SELECT BIDIRECTIONNEL
        d3.selectAll('[target="'+selected+'"][other="#blop1.5"]').transition().duration(200).style("opacity", 1);
        d3.selectAll('[included-group="'+selected+'"]').transition().duration(200).style("opacity", 1);
    }
    function unhiglight_all(node,i){
        ...
        d3.selectAll('[included-group="'+selected+'"]').transition().duration(200).style("opacity", 0.7);
    }
});
